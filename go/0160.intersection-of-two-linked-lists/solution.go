// Created by Vintingb at 2023/04/27 04:46
...
— 请注意相交节点的值不为 1，因为在链表 A 和链表 B 之中值为 1 的节点 (A 中第二个节点和 B
中第三个节点) 是不同的节点。换句话说，它们在内存中指向两个不同的位置，而链表 A 和链表 B 中值为 8 的节点 (A
...
- 如果 `listA` 和 `listB` 有交点， `intersectVal == listA[skipA] ==
listB[skipB]`
